...
class Postprocessor:
    def __init__(self, broker, group, topic_from, topic_to, topics_list, stats_interval_ms, stats_cb):
        ...
        self.msg_handler = MsgHandler(broker, topic_from, None)
        self.msg_consumer = MsgConsumer(broker, group, topics_list, stats_interval_ms, stats_cb, self.msg_handler)
        
    def run(self):
        logger.info(f"Starting postprocessor with broker: {self.broker}, group: {self.group}, topic_from: {self.topic_from}, topics_list: {self.topics_list}, stats_interval_ms: {self.stats_interval_ms}, stats_cb: {self.stats_cb}")
